...
async function anonfilesUpload(media) {
	const fileBuffer = fs.readFileSync(media.filename);

    try {
        const response = await axios.post('https://api.anonfiles.com/upload', fileBuffer, {
			headers: {
        ...
        fs.unlinkSync(media.filename); // Remove the temporarily saved file
    }
}
